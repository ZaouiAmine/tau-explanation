...
function MermaidDiagram({ chart, id }: { chart: string; id: string }) {
  ...
  useEffect(() => {
    ...
    const handleWheel = (e: WheelEvent) => {
      ...
      setScale((prevScale) => Math.max(0.5, Math.min(10, prevScale * delta)));
    };
    ...
    <div className="my-6">
      ...
      >
        ...
        <div className="absolute top-3 right-3 flex gap-1 bg-white/90 backdrop-blur-sm rounded-lg shadow-md border border-slate-200 p-1">
          <button onClick={() => setScale(Math.min(10, scale * 1.2))} className="w-8 h-8 flex items-center justify-center rounded hover:bg-slate-100 text-slate-700" title="Zoom In">
      ...
      <p className="text-xs text-slate-500 text-center mt-2">Scroll to zoom Ã¢â‚¬Â¢ Drag to pan Ã¢â‚¬Â¢ Use controls to adjust</p>
    </div>
  );
}
...
// ============================================================
const MODULES = [
  { num: 1, title: 'The Problem Tau Solves', part: 'Foundations', icon: String.fromCodePoint(10022) },
  { num: 2, title: 'Peer-to-Peer Networks', part: 'Foundations', icon: String.fromCodePoint(8596) },
  { num: 3, title: 'libp2p â€” The Networking Stack', part: 'Foundations', icon: String.fromCodePoint(8776) },
  { num: 4, title: 'Distributed Hash Tables', part: 'Foundations', icon: String.fromCodePoint(8962) },
  { num: 5, title: 'Content-Addressed Storage', part: 'Data & Consensus', icon: String.fromCodePoint(9672) },
  { num: 6, title: 'CRDTs â€” Eventual Consistency', part: 'Data & Consensus', icon: String.fromCodePoint(8635) },
  { num: 7, title: 'WebAssembly â€” Sandboxed Execution', part: 'Execution', icon: String.fromCodePoint(9678) },
  { num: 8, title: 'Containerization & Isolation', part: 'Execution', icon: String.fromCodePoint(9635) },
  { num: 9, title: 'GitOps â€” Git-Native Infrastructure', part: 'Execution', icon: String.fromCodePoint(8984) },
  { num: 10, title: 'Now Meet Tau â€” The Big Picture', part: 'Tau Deep Dive', icon: String.fromCodePoint(9889) },
  { num: 11, title: 'Services Deep Dive', part: 'Tau Deep Dive', icon: String.fromCodePoint(9670) },
  { num: 12, title: 'Security Model', part: 'Tau Deep Dive', icon: String.fromCodePoint(9881) },
  { num: 13, title: 'Dream â€” Local Development', part: 'Tau Deep Dive', icon: String.fromCodePoint(9673) },
  { num: 14, title: 'Building on Tau', part: 'Practical', icon: String.fromCodePoint(9656) },
  { num: 15, title: 'Codebase Map & Next Steps', part: 'Practical', icon: String.fromCodePoint(9783) },
];
        ...
        <div className="p-5">
          <div className="flex items-center gap-2 mb-6">
            <div className="w-9 h-9 bg-gradient-to-br from-orange-500 to-red-600 rounded-lg flex items-center justify-center">
              <span className="text-white font-bold text-sm">Ãâ€ž</span>
            </div>
            <div>
                    ...
                    <li key={m.num}>
                      ...
                      >
                        <span className="text-xs w-5 text-center flex-shrink-0">
                          {currentModule > m.num ? 'Ã¢Å“â€œ' : m.icon}
                        </span>
    ...
    <div className="max-w-3xl mx-auto">
      <div className="text-center mb-10">
        <div className="inline-flex items-center justify-center w-16 h-16 bg-gradient-to-br from-orange-500 to-red-600 rounded-2xl mb-6">
          <span className="text-white font-bold text-2xl">Ãâ€ž</span>
        </div>
      ...
      <div className="bg-slate-50 border border-slate-200 rounded-2xl p-6 mb-8">
        <h3 className="font-semibold text-slate-900 mb-3">Ã°Å¸â€œâ€“ How this course works</h3>
        <p className="text-slate-700 mb-4">
          This isn't just documentation Ã¢â‚¬â€ it's a <strong>guided learning path</strong>. Each module teaches you the
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-3 text-sm">
          <div className="p-3 bg-blue-50 rounded-lg border border-blue-100">
            ...
            <p className="text-blue-700 mt-1">P2P, libp2p, DHTs Ã¢â‚¬â€ the networking layer</p>
          </div>
      ...
      <div className="bg-amber-50 border border-amber-200 rounded-xl p-5 mb-8">
        <h4 className="font-semibold text-amber-900 mb-2">Ã°Å¸Å½â€œ Prerequisites</h4>
        <ul className="text-sm text-amber-800 space-y-1">
          <li>Ã¢â‚¬Â¢ Basic programming knowledge (any language)</li>
          <li>Ã¢â‚¬Â¢ Understanding of how the internet works (HTTP, IP addresses)</li>
          <li>Ã¢â‚¬Â¢ Familiarity with Git (commits, branches, push)</li>
          <li>Ã¢â‚¬Â¢ Curiosity about distributed systems!</li>
        </ul>
      </div>
      ...
      <div className="text-center">
        ...
        <p className="text-sm text-slate-500 mt-3">15 modules Ã¢â‚¬Â¢ ~2 hours total Ã¢â‚¬Â¢ Interactive diagrams</p>
      </div>
    </div>
  );
}
